...
def Check_kkt(n_buses):
    
    simulation_parameters = create_example_parameters(n_buses)
    P_Loads=pd.read_csv('Data_File/'+str(n_buses)+'/NN_input.csv').to_numpy()
    P_Gens =pd.read_csv('Data_File/'+str(n_buses)+'/NN_output.csv').to_numpy()
    ...
    for l in range(np.size(P_Loads,0)):
        ...
        # The number of variables (columns) must remain exactly the same
        num_vars = 2*n_gbus+2*n_line+1
        
        # The absolute maximum possible number of equations (rows) generated by the loops
        max_rows = 3*n_gbus+2*n_line
